...
class LinkedHashSetCustom<E> {
	...
	/**
	 * Method displays all objects in LinkedHashSetCustom. insertion order is
	 * not guaranteed, for maintaining insertion order refer LinkedHashSet.
	 */
	public void display() {
	...
	/**
	 * Method removes object from setCustom. insertion order is not guaranteed,
	 * for maintaining insertion order refer LinkedHashSet.
	 * 
	 * @param obj
	 */
 ...
 *         implementation of LinkedHashMap(without using java api's)- which
 *         allows us to store data in key-value pair form. It maintains
 *         insertion order, uses DoublyLinkedList for doing so. If key which
 *         already exists is added again, its value is overridden but insertion
 *         order does not change, BUT, if key-value pair is removed and value is
 *         again added than insertion order changes(which is quite natural
 *         behaviour).
 * @param <K>
 * @param <V>
 */
